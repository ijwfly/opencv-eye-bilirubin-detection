...
original_image = cv2.imread('test_images/front.jpg', cv2.IMREAD_COLOR)


# detect eye
original_image = cv2.cvtColor(original_image, cv2.COLOR_RGB2GRAY)
faces = cv2.CascadeClassifier('classifiers/haarcascade_eye.xml')
detected = faces.detectMultiScale(original_image, 1.3, 5)

pupilFrame = original_image.copy()
pupilO = original_image.copy()
...
for x, y, w, h in detected:
    ...
    cv2.line(original_image, (x, y), (x + w, y + h), (0, 0, 255), 1)
    cv2.line(original_image, (x + w, y), (x, y + h), (0, 0, 255), 1)
    pupilFrame = cv2.equalizeHist(original_image[y + int(h * .25):(y + h), x:(x + w)])
    pupilO = pupilFrame
